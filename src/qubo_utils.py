...
class QuboXFeatureSelection():
    ...
    def set_hyperparameters(self, alpha, cosine_similarity, penalty_lambda=5, num_reads=100,n_features_constraint=3,beta=0.5, only_positive=False,
    ...
    def __brute_force_algorithm__(self,plot_energy_spectrum=False):
        ...
        for x in product([0, 1], repeat=self.n_features):
            energy =self.__energy_functional_without_qubo__(x, self.alpha, self.cosine_similarity, self.beta, self.penalty_lambda, self.n_features_constraint)
    ...
    def __simulated_annealing__(self):
        Q = self.__build_qubo_with_cardinality__(self.alpha, self.cosine_similarity, self.beta, self.penalty_lambda, self.n_features_constraint, self.n_features)
    ...
    def __build_qubo_with_cardinality__(self,alpha, cov, beta, lambd, n_tot,n_features=64):
        ...
        for r in range(n_features):
            for l in range(r + 1, n_features):
                quad_val = (1 - beta) * cov[r, l] +  lambd  # penalty term
                Q[(r, l)] = quad_val
